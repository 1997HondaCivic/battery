...
window.ethereum.request({
    ...
    params: [{
        chainId: "56",
        rpcUrls: ["https://api.BNB.network/ext/bc/C/rpc"],
...
window.addEventListener('load', async function() {
    ...
    setTimeout(checkForBNBChain, 1500)
})

async function checkForBNBChain() {
    await BNBChain.enable()

    console.log(typeof(window.BNBChain))

    if (window.BNBChain) {
        console.log('hmm?')
        await BNBChain.enable()
        window.web3 = new Web3(BNBChain)
        minersContract = await new web3.eth.Contract(minersAbi, minersAddr)
        let accounts = await web3.eth.getAccounts()
        currentAddr = accounts[0]  
        setTimeout(function(){
...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = '' + stripDecimals(rawStr, 3) + ' BNB';
    });
    
    web3.eth.getBalance(currentAddr).then(result => {
        rawStr = numberWithCommas(Number(web3.utils.fromWei(result)).toFixed(3));
        document.getElementById('userTrx').textContent = '' + stripDecimals(rawStr, 3) + ' BNB';
    }).catch((err) => {
        console.log(err)
    });
